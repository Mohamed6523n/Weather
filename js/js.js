...
function GetCont(cunt) {

    var httpreq = new XMLHttpRequest();
    httpreq.open('GET', `https://api.weatherapi.com/v1/forecast.json?key=7d77b96c972b4d119a3151101212704&q=${cunt}&days=3`)
    httpreq.send()

    httpreq.addEventListener('readystatechange', function () {

        if (httpreq.readyState == 4 && httpreq.status == 200) {
            let x = JSON.parse(httpreq.response)
            data = x
            display()
        }
    })
...
function display() {
    ...
    <div class="col-md-4 item-2 text-center border border-1 border-dark rounded rounded-1  mt-5">
        <div class="inner-item-2  main-color ">
            <p>Sunday</p>
        </div>
